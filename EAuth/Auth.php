...
require 'src/PHPMailer/src/OAuth.php';
require 'src/PHPMailer/src/POP3.php';
...
/**
 * This is Auth class.
 * It requires PHP version 5.6 and above.
 */
class Auth
{
	...
	public function __construct(\PDO $pdo, Config $config)
	{
		if (version_compare(phpversion(), '5.6.0', '<')) {
		    die('PHP version 5.6.0 and above is required!');
		}
	...
	/**
	* Creates a new user, adds them to the database.
	* @param string $email
	* @param string $password
	...
	* @return array $return
	*/
	public function register($email, $password, $repeatpassword, $params = [], $privacy , $captcha_response = null, $use_email_activation = null)
	{
		$return['error'] = true;
		...
		if ($privacy != 0 && $privacy != 1) {
			$this->addAttempt();
			$return['message'] = $this->__lang("privacy_invalid");
		}

		if ($privacy == 0 ) {
			$return['message'] = $this->__lang('Please read and agree with our Privacy and Policy');
	...
	/**
	* Gets User ID for a given email address, return int
	* @param string $email
	* @return int $uid
	*/
	public function getUID($email)
	{
	    ...
	    $query_prepared = $this->pdo->prepare($query);
	...
	*/
	...
	/**
	* Allow users to reset their password after requesting a reset key.
	* @param string $key
	* @param string $password
	* @param string $repeatpassword
	* @param string $captcha_response = null
	...
	*/
	...
	/**
	* Gets basic user data for a given User ID and returns an array of that data
	* @param int $uid
	* @return array $data
	*/
	...
	/**
	* This informs if a user is locked out
	* @return string
	*/
	public function isBlocked()
	{
	    ...
	    $query_prepared = $this->pdo->prepare($query); // INET_ATON(:ip)
	...
	*/
	...
	/**
	* Removes all existing sessions for a given User ID
	* @param int $uid
	* @return boolean
	*/
	...
	*/
	...
	/**
	* Retrieves the User ID associated with a given session hash
	* @param string $hash
	* @return int $uid
	*/
	...
	*/
	protected function addUser($email, $password, $params = array(), &$use_email_activation)
	{
		...
		$uid = $this->pdo->lastInsertId("{$this->config->table_users}_id_seq");
	...
	*/
	...
	public function deleteUserForced($uid)
	{
	    ...
	    $return['error'] = false;
	    $return['message'] = $this->__lang("account_deleted2");
	...
	*/
	...
	{
	    if (! $this->pdo->query("SHOW TABLES LIKE '{$this->config->table_emails_banned}'")->fetchAll() ) {
	        return false;
	    };
	...
	*/
	...
	public function updateUser($uid, $params)
	{
	    ...
	    //NB: There is NO possible SQL-injection in here, 'cause $setParams will be like 'name = ?, age = ?'
